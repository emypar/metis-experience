...
def mnist_blackbox(train_start=0, train_end=60000, test_start=0,
                   ...
                   nb_epochs_s=10, lmbda=0.1, eps=0.3):
    """
    ...
    X_train_adv = fgsm.generate_np(X_train, **fgsm_par)
    X_test_adv = fgsm.generate_np(X_test, **fgsm_par)
    ...
    return accuracies, X_train, Y_train, X_test, Y_test, X_train_adv, X_test_adv


def main(argv=None):
    accuracies, X_train, Y_train, X_test, Y_test, X_train_adv, X_test_adv = \
        mnist_blackbox(nb_classes=FLAGS.nb_classes, batch_size=FLAGS.batch_size,
                       ...
                       lmbda=FLAGS.lmbda,
                       eps=FLAGS.eps,
                       train_start=FLAGS.train_start, train_end=FLAGS.train_end,
                       test_start=FLAGS.test_start, test_end=FLAGS.test_end,
                       )
    np.savez(
        'data/mnist/adv_set-{:.03f}-{:.03f}-{}-{}-{}-{}'.format(
            FLAGS.lmbda, FLAGS.eps, 
            FLAGS.train_start, FLAGS.train_end,
            FLAGS.test_start, FLAGS.test_end
        ),
        X_train=X_train, Y_train=Y_train, 
        X_test=X_test, Y_test=Y_test, 
        X_train_adv=X_train_adv, X_test_adv=X_test_adv,
        accuracies=(accuracies['bbox'], accuracies['bbox_on_sub_adv_ex']),
        flags=(FLAGS.lmbda, FLAGS.eps)
    )
...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer('train_start', 0, 'Train set start index')
    flags.DEFINE_integer('train_end', 60000, 'Train set start index')
    flags.DEFINE_integer('test_start', 0, 'Test set start index')
    flags.DEFINE_integer('test_end', 10000, 'Test set start index')
    ...
    flags.DEFINE_float('eps', 0.3, 'Epsilon from arxiv.org/abs/1602.02697')
